...
public sealed record AreaDaCulturaNoGrupo(
    string CulturaCodigo, string CulturaNome, decimal? AreaHectares, decimal? HectaresPorMaquina);
...
/// <param name="CulturaDominante">A cultura cujos parâmetros foram usados; nula quando não há.</param>
...
public static class PotencialEstrutural
{
    ...
    {
        var comArea = culturas.Where(c => c.AreaHectares is > 0).ToList();
        ...
        var dominante = comArea.MaxBy(c => c.AreaHectares!.Value)!;
        var areaUtil = dominante.AreaHectares!.Value;
        var outras = comArea.Where(c => c.CulturaCodigo != dominante.CulturaCodigo).Select(c => c.CulturaNome).ToList();

        var maquinas = dominante.HectaresPorMaquina is > 0 ? areaUtil / dominante.HectaresPorMaquina!.Value : (decimal?)null;

        return new ParqueDoGrupo(maquinas, areaUtil, dominante.CulturaNome, outras);
    }
