...
public sealed partial class EsquemaENomenclaturaTestes
{
    ...
    public void Os_oito_schemas_do_modelo_unificado_existem_e_somam_cinquenta_e_cinco_tabelas()
    {
        ...
        porSchema.Should().BeEquivalentTo(esperado,
            "a conta é 75 tabelas de modelo em 8 schemas: a fase 1 (documento 41) trouxe 80 em 10 " +
            ...
            "cinco da estrutura agropecuária a 66 as duas dos preços de mercado a 67 a dos custos de produção a 68 as duas do crédito rural do SICOR e a 71 as duas dos parâmetros do potencial com vigência (os gerais e a percepção do gestor) a 136 as quatro das integrações configuráveis (conexão, verificação, rotina e execução da rotina) e a 154 o de-para entre a chave de cada fonte e o município do catálogo, e a 155 o total que o IBGE publica para o estado nas quatro pesquisas da estrutura agropecuária, e a 156 o milho separado em 1ª e 2ª safra, e a 165 as quatro do catálogo de culturas e categorias de máquina, e a 160 as duas do compartilhamento de máquina entre culturas. O portão continua o mesmo nos dois sentidos: mudar " +
            "este número exige a decisão da seção 10.2 e a atualização do documento 14, seção 2.1, " +
            "na MESMA mudança");

        porSchema.Values.Sum().Should().Be(75);
    }
