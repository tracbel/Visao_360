/* ==============================================================
   Objeto ..........: dbo.X_BI_PROSPECCAO_MAQUINAS_FATURAMENTO
   Tipo ............: SQL_STORED_PROCEDURE
   Criado em .......: 2023-03-10 10:49:05
   Modificado em ...: 2023-03-10 10:59:06
   Linhas ..........: 150
...
--Executar procedure 
--Exec X_BI_PROSPECCAO_MAQUINAS_FATURAMENTO;


CREATE   Procedure [dbo].[X_BI_PROSPECCAO_MAQUINAS_FATURAMENTO]  
as


----------------------------------------------------------------------------------------------------------
--Tabela 01
IF  EXISTS (SELECT * FROM tempdb.sys.tables WHERE name like '#Tab01_2%')
BEGIN
   DROP TABLE #Tab01_2
END
                    ...
                    Create nonclustered index idx on #Tab01_2	 (PROCESSO)	
                    --SELECT * FROM #Tab01	
----------------------------------------------------------------------------------------------------------
--Tabela 02
IF  EXISTS (SELECT *FROM tempdb.sys.tables WHERE name like '#Tab02_2%')
BEGIN
   DROP TABLE #Tab02_2
END 
					...
					INNER JOIN 		#Tab01_2 		aax ON aax.PROCESSO		= ax.PROCESSO
		            ...
		            Create nonclustered index idx on #Tab02_2	 (SEQPESSOAPRC)
...
----------------------------------------------------------------------------------------------------------
--Tabela 03					
IF  EXISTS (SELECT *FROM tempdb.sys.tables WHERE name like '#Tab03_2%')
BEGIN
   DROP TABLE #Tab03_2
END 					
				
    SELECT --DISTINCT
        'FATURAMENTO'					as classe,
    ...
    INNER JOIN		#Tab02_2 			kx 	ON kx.SEQPESSOAPRC 	= k.SEQPESSOAPRC
