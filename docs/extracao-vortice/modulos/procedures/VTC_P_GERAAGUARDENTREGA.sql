/* ==============================================================
   Objeto ..........: dbo.VTC_P_GERAAGUARDENTREGA
   Tipo ............: SQL_STORED_PROCEDURE
   Criado em .......: 2025-09-11 17:56:04
   Modificado em ...: 2026-03-11 12:31:16
   Linhas ..........: 258
   Escreve em tabela: SIM (INSERT)
   Alvos de escrita : GEP_IMPORT, GEP_IMPORTAPROVACAO, EXT_AGUARDENTREGA
   Tabelas referidas: EXT_AGUARDENTREGA, GE_USUARIO, GEP_IMPORT, GEP_IMPORTAPROVACAO, IV_HISTORICO, IV_OPERADOR, IV_PROCDADO
   Outras refs .....: IV_Q$PEDIDO_SAM, PRC_GET_SEQUENCIA_TABELA
   Fonte: banco CRM (Vortice CRM / Tracbel) - extracao somente leitura
   ============================================================== */

CREATE   PROCEDURE [dbo].[VTC_P_GERAAGUARDENTREGA] 
AS
BEGIN
    SET NOCOUNT ON;
    ...
    -- Cursor para iterar sobre os registros da IV_HISTORICO com resultado 3663 e dentro do período
...
---	JOIN IV_AGENDA AGE ON AGE.PROCESSO = H.PROCESSO AND AGE.REALIZADA = 'N'
    JOIN IV_Q$PEDIDO_SAM V ON H.PROCESSO = V.PROCESSO AND V.QUANTIDADE>0
    ...
    WHERE 1=1
      AND H.RESULTADO = 3663  --- Resultado Venda Direta sAM , da açao 768 Vender Equipamentos Novos
	  AND NOT EXISTS ( SELECT 1 FROM EXT_AGUARDENTREGA WHERE PROCESSODNA = ppd.PROCESSODNA  )
...
----			set @DETALHE = @DETALHE + + 'Tipo: ' + @TIPO + '  Marca: ' + @MARCA + '  Modelo: ' + @MODELO
		...
		--- pega sequencia AGENDA
            ...
            VALUES (
				...
				CAST ( @NROEMPRESA AS varchar)  + @VS + CAST( @CODPROCESSO AS VARCHAR) + @VS + '823' + @VS + 'Aguardando Entrega Equipamento (FY25)' + @VS + 
				'Pedido SAM' + @VS + 'X' + @VS + 
            ...
            SET @i = @i + 1;
        END
		INSERT INTO EXT_AGUARDENTREGA ( PROCESSO, PROCESSODNA, DTAGERACAO ) VALUES ( @vnProcesso,  @PROCESSODNA, GETDATE() )
