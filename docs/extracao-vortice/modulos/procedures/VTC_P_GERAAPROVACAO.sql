/* ==============================================================
   Objeto ..........: dbo.VTC_P_GERAAPROVACAO
   Tipo ............: SQL_STORED_PROCEDURE
   Criado em .......: 2025-06-27 10:32:56
   Modificado em ...: 2026-03-11 12:29:28
   Linhas ..........: 252
   Escreve em tabela: SIM (INSERT)
   Alvos de escrita : GEP_IMPORT, GEP_IMPORTAPROVACAO, EXT_APROVACAO
   Tabelas referidas: EXT_APROVACAO, GE_USUARIO, GEP_IMPORT, GEP_IMPORTAPROVACAO, IV_HISTORICO, IV_OPERADOR, IV_PROCDADO
   Outras refs .....: IV_Q$VENDA_EQUIPAMENTO, PRC_GET_SEQUENCIA_TABELA
   Fonte: banco CRM (Vortice CRM / Tracbel) - extracao somente leitura
   ============================================================== */

CREATE PROCEDURE [dbo].[VTC_P_GERAAPROVACAO]
AS
BEGIN
    SET NOCOUNT ON;
    ...
    -- Cursor para iterar sobre os registros da IV_HISTORICO com resultado 3231 e dentro do período
    DECLARE CURHIS CURSOR FOR
    SELECT H.PROCESSO, 
		   H.SEQPESSOA, 
		   H.NROEMPRESA,
		   V.VENDA_QTDE_EQUIPAMEN,
		   ...
		   V.VENDA_MODELO_EQUIPAM AS MODELO,
		   ATD.Gerente,
		   USU.SEQUSUARIO AS SEQGERENTE
    FROM IV_HISTORICO H
---	JOIN IV_AGENDA AGE ON AGE.PROCESSO = H.PROCESSO AND AGE.REALIZADA = 'N'
    JOIN IV_Q$VENDA_EQUIPAMENTO V ON H.PROCESSO = V.PROCESSO AND V.VENDA_QTDE_EQUIPAMEN>0
    ...
    WHERE 1=1
      AND H.RESULTADO = 3231  --- Pedido realizado , da açao 768 Vender Equipamentos Novos
	  AND NOT EXISTS ( SELECT 1 FROM EXT_APROVACAO WHERE PROCESSODNA = ppd.PROCESSODNA  )
	  	  AND H.DTAINCLUSAO > DATEADD(HOUR, -2,  '2025-07-24 07:00:00' )
		  ;
	...
	OPEN CURHIS;
    FETCH NEXT FROM CURHIS INTO  @Processo, @SeqPessoa, @NROEMPRESA, @Quantidade, @VALOR, @USUINCLUSAO, @PROCESSOPAI, 
		  ...
		  @TIPO, @MARCA , @MODELO , @GERENTE, @SEQGERENTE
    ...
    BEGIN
		...
		--- pega sequencia PROCESSO
			...
			set @DETALHE = @DETALHE + + 'Tipo: ' + @TIPO + '  Marca: ' + @MARCA + '  Modelo: ' + @MODELO
			...
			begin
		            ...
		            VALUES (
						...
						GETDATE(),
						'PROCESSO; DESCRICAO; USUINCLUSAO; DTAINCLUSAO; VALOR; QTDE; USURESPONSAVEL; DTAALTERACAO; USUALTERACAO; REALIZADO; DTAFASE; FASE; STATUS;'+
						'PRIORIDADE',
						...
						CAST (@VALOR AS VARCHAR) + @VS + CAST( @Quantidade AS VARCHAR) + @VS + isnull(@GERENTE, '') + @VS + 
						'CONVERT( DATETIME, ' + CHAR(39) + format ( @DTAGERACAO, 'dd-MM-yyyy HH:mm:ss') + CHAR(39)  + ', 103)' + @VS + 
						@USUINCLUSAO + @VS + '0' + @VS + 
						'CONVERT( DATETIME, ' + CHAR(39) + format ( @DTAGERACAO, 'dd-MM-yyyy HH:mm:ss') + CHAR(39)  + ', 103)' + @VS + 
		            ...
		            VALUES (
						...
						isnull(@VENDEDOR, '') + @VS + ISNULL(@FORMAPRIMCONT, '') + @VS + ISNULL(@ATIVORECEPTIVO, '') + @VS + ISNULL(@MOTIVO, '') + @VS + @ORIGEM + 
		...
		--- pega sequencia AGENDA
            ...
            VALUES (
				...
				GETDATE(),
				'SEQAGENDA;SEQUSUARIO; SEQPESSOA; TIPOAGENDAMENTO; TAREFACOMPROMISSO; NROEMPRESA; CODPROCESSO; ACAO; ASSUNTO; CLASSE; DTAAGENDA; DTAAGENDAORIGINAL; PRIORIDADE; DETALHE; HISTORICOORIGEM; USUGEROUACAO; DTAGERACAO; REALIZADA; VENDEDOR; PROCESSO; DTAGERACAOORIG; STATUS',
				CAST ( @vnSeqagenda as varchar) + @VS + 
				CAST ( isnull(@SEQGERENTE, @SEQUSUARIO) as varchar) + @VS + CAST ( @SeqPessoa AS VARCHAR) + @VS + 'A' + @VS + 'T' + @VS + 
				CAST ( @NROEMPRESA AS varchar)  + @VS + CAST( @CODPROCESSO AS VARCHAR) + @VS + '769' + @VS + 'Aprovar Venda (FY25)' + @VS + 
				'X' + @VS + 
            ...
            SET @i = @i + 1;
        END
		INSERT INTO EXT_APROVACAO ( PROCESSO, PROCESSODNA, DTAGERACAO ) VALUES ( @vnProcesso,  @PROCESSODNA, GETDATE() )
