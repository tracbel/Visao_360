/* ==============================================================
   Objeto ..........: dbo.PR_INT_NFS
   Tipo ............: SQL_STORED_PROCEDURE
   Criado em .......: 2012-05-24 17:31:40
   Modificado em ...: 2016-12-08 19:21:38
   Linhas ..........: 205
...
CREATE procedure [dbo].[PR_INT_NFS] 
AS
	...
	WHERE NFS.TIPOPECOD = 3
	  ...
	  AND SUBSTRING(NFS.CANFSADATEMISSAO FROM 1 FOR 4) = EXTRACT( YEAR FROM CURRENT_DATE)
	  AND SUBSTRING(NFS.CANFSADATEMISSAO FROM 5 FOR 2) = EXTRACT( MONTH FROM CURRENT_DATE)
	  AND SUBSTRING(NFS.CANFSADATEMISSAO FROM 7 FOR 2) >= EXTRACT( DAY FROM CURRENT_DATE)-1
	') XA
	  --AND SUBSTRING(NFS.CANFSADATEMISSAO FROM 1 FOR 4) >= EXTRACT( YEAR FROM CURRENT_DATE-1)
	  --AND SUBSTRING(NFS.CANFSADATEMISSAO FROM 5 FOR 2) >= EXTRACT( MONTH FROM CURRENT_DATE-1)
	  --AND SUBSTRING(NFS.CANFSADATEMISSAO FROM 7 FOR 2) >= EXTRACT( DAY FROM CURRENT_DATE-1)
