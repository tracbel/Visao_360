...
| R-5 | **A integração roda no servidor** (serviço do Windows ou rotina da aplicação). Nunca um script de estação gravando no banco central | doc 35 §6.2, §11. **20/09/2026 (#64, #65):** as fontes públicas saíram da exceção — `--somente-pam` e `--somente-estrutura` rodam no servidor, na tarefa anual `TracbelCrmFontesPublicas` (doc 32 §8.3.3), e ficou MEDIDO que o servidor alcança a internet. Continua de exceção o que depende das duas planilhas do comercial, em `scripts/deploy/carregar-territorio-no-servidor.ps1` |
